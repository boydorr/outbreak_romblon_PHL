...
string=readDNAStringSet("output/phylogenetic/concatenated_alignment/ph_all_merged_by_id_746.fasta")
...
write.table(wgs.names, file = "output/phylogenetic/wgs_alignment/wgs.names.746.txt", sep = "", row.names=F, quote=F)

## wgs associated metadata
all_phl.annot=read.csv("output/phylogenetic/concatenated_alignment/ph_metadata_merged_by_id_746.csv")
wgs.metadata=all_phl.annot[which(all_phl.annot$Sample.ID %in% wgs.names$Accession),]
write.csv(wgs.metadata, file = "output/phylogenetic/wgs_alignment/wgs.metadata.746.csv", row.names=F)

## root the tree with all philippines cases
all_tree=read.tree("output/phylogenetic/trees/ph_all_746_ft.nwk")
...
write.table(all.dates, file = "output/phylogenetic/trees/all.dates.746.txt", row.names=F, sep="\t", quote = F,col.names = F)
...
rooted$edge.length=rooted$edge.length/11925
write.tree(rooted,"output/phylogenetic/trees/all_rooted.746.nwk")
# outside of R- use gotree prune to extract WGS only tree 

# create dates file for lsd2
# important - lsd2 in R requires first line to be number of sequences
wgs.metadata <- read.csv("output/phylogenetic/wgs_alignment/wgs.metadata.746.csv")
...
write.table(wgs.dates, file = "output/phylogenetic/wgs_alignment/wgs.dates.746.txt", row.names=F, sep="\t", quote = F,col.names = F)
...
write.csv(wgs.dates, file = "output/phylogenetic/wgs_alignment/wgs.clockor2.746.csv", row.names=F, quote = F)

## import the wgs tree and root according to best temporal root 
### trees
wgs.tree=read.tree("output/phylogenetic/wgs_alignment/ph_wgs_ft.nwk")
...
r=roottotip(rooted,as.numeric(ordered.dates2))
rooted$edge.length=rooted$edge.length/11925
write.tree(rooted,"output/phylogenetic/wgs_alignment/wgs_rooted.746.nwk")


result=lsd2(inputTree="output/phylogenetic/wgs_alignment/wgs_rooted.746.nwk", inputDate ="output/phylogenetic/wgs_alignment/wgs.dates.746.txt", outFile="output/phylogenetic/wgs_alignment/wgstest_746", seqLen=11925, ZscoreOutlier=3)

result2=lsd2(inputTree="output/phylogenetic/wgs_alignment/wgs_rooted.746.nwk", inputDate ="output/phylogenetic/wgs_alignment/wgs.dates.746.txt", outFile="output/phylogenetic/wgs_alignment/wgs_rtt_wgs_746", seqLen=11925, ZscoreOutlier=3)
...
all.result=lsd2(inputTree="output/phylogenetic/trees/all_rooted.746.nwk", inputDate ="output/phylogenetic/trees/all.dates.746.txt", outFile="output/phylogenetic/wgs_alignment/all_wgsrate_lsd_CI_746", seqLen=11925, ZscoreOutlier=5, givenRate=rate, confidenceInterval=1000)
