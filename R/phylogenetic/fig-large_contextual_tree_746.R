...
### trees
# time scaled
all_phl.tree.time=read.beast("output/phylogenetic/dated_trees/all_wgsrate_lsd_CI_746.date.nexus")
# substitution scaled
all_phl.tree.subs=read.newick("output/phylogenetic/trees/all_rooted.746.nwk")
...
# highlight Mindoro seq- new and old
...
tiff("figures/S1_contextTrees_300dpi.tif", units = "in", width=11.69, height=8.27, res= 300, compression = "lzw")     
trees+ plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text(size = 12))
dev.off()

pdf("figures/S1_contextTrees.pdf", width=11.69, height=8.27)     
trees+ plot_annotation(tag_levels = 'A')& 
  theme(plot.tag = element_text(size = 12))
dev.off()
