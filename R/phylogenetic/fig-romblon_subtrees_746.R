...
## DATA#################
### trees
all_phl.tree=read.beast("output/phylogenetic/pastml_analysis/all_mppa_province_746/named.tree_all_wgsrate_lsd_CI_746.date.nexus")
...
tiff("output/figures/phylogenetic/copy_final_phylo_300dpi_panellab.tif", units = "in", width=11.69, height=8.27, res= 300, compression = "lzw")
legends/plot.part1+ plot_layout(heights = c(0.2,1)) +
  plot_annotation(tag_levels = list(c('', '', 'A1','A2','B1','B2','C1','C2','D')))&
  theme(plot.tag = element_text(size = 12))
dev.off()

pdf("output/figures/phylogenetic/copy_final_phylo_panellab2.pdf", width=11.69, height=8.27)
legends/plot.part1+ plot_layout(heights = c(0.2,1)) +
  plot_annotation(tag_levels = list(c('', '', 'A1','A2','B1','B2','C1','C2','D')))&
  theme(plot.tag = element_text(size = 12))
dev.off()

jpeg("output/figures/phylogenetic/copy_final_phylo_panellab2.jpg", quality=100)
legends/plot.part1+ plot_layout(heights = c(0.2,1)) +
  plot_annotation(tag_levels = list(c('', '', 'A1','A2','B1','B2','C1','C2','D')))&
  theme(plot.tag = element_text(size = 12))
dev.off()
