...
# filter trees to those for scenario: epi data only / barangay centroids / pruned w distance and time cut-offs both 0.99
links_consensus <- links_consensus[scenario == 9]
ttrees <- ttrees[scenario == 9]
...
links_consensus %>%
  ...
  arrange(id_case) %>%
  fwrite(here("output/figures/epi_only_barangay_centroids_prunedDT99/constree_epionly_prunedDT99_chainids.csv"))
...
tree_col_by_membership_no_unsampled_nodes <- 
  plot_lin_ts(links_consensus, case_dates,
              colour_by = "membership",
              colour_edges = TRUE,
              shape_unsampled = NA,
              id_labels = FALSE,
              timeline_breaks = 3)
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/constree_epionly_prunedDT99_nonodes.jpeg"), tree_col_by_membership_no_unsampled_nodes, height = 8, width = 8)
...
tree_col_by_membership_id_labels <- 
  plot_lin_ts(links_consensus, case_dates,
              ...
              id_labels = TRUE,
              timeline_breaks = 3)
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/constree_epionly_prunedDT99_idlabels.jpeg"), tree_col_by_membership_id_labels, height = 8, width = 8)
...
tree_col_by_membership_grey_edges <- 
  plot_lin_ts(links_consensus, case_dates,
              ...
              id_labels = NA,
              timeline_breaks = 3)
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/constree_epionly_prunedDT99_greyedges.jpeg"), tree_col_by_membership_grey_edges, height = 8, width = 8)
...
epicurve <- plot_epicurve(links_consensus, case_dates,
                          colour_by = "membership",
                          timeline_breaks = 3)
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/epicurve_consensus_epionly_prunedDT99.jpeg"), epicurve, height = 8, width = 8)
...
case_data <-
  animal_cases %>%
  mutate(utm_easting = utm_easting_centroid,
...
map_w_cases <- plot_map_w_lin_cases(links_consensus, case_coords = case_data,
                                    map = Romblon_shp,
                                    colour_by = "membership")
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/map_consensus_epionly_prunedDT99.jpeg"), map_w_cases, height = 8, width = 8)
...
#(Relative sizing of the Figure tags isn't correct...)
ggsave(here("output/figures/epi_only_barangay_centroids_prunedDT99/consensus_epicurve_trees_map.jpeg"), tm_trees_main_figure, height = 8, width = 8)
...
saveVideo({ 
  ...
  video.name = here("output/figures/epi_only_barangay_centroids_prunedDT99/tmtree_animation.mp4"),
